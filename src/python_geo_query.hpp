...
namespace boost_geo_query
{
    ...
    public:
        ...
        virtual QueryResults nearest_to(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) = 0;
        virtual QueryResults intersects_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) = 0;
        virtual QueryResults overlaps_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) = 0;
        virtual QueryResults within_distance_of(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type, Distance d) = 0;
    };
    ...
    public:
        ...
        BoostGeoQueryWrapper(const InputPoints &xy, const IndexVector &rowPtr)
        {
        ...
        QueryResults nearest_to(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type)
        {
        ...
        QueryResults intersects_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type)
        {
            if (type == accepted_input_types::point)
            {
        ...
        QueryResults overlaps_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type)
        {
            if (type == accepted_input_types::point)
            {
        ...
        QueryResults within_distance_of(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type, Distance d)
        {
            if (type == accepted_input_types::point)
            {
    ...
    private:
        ...
        std::vector<U> _convert_geometry_for_query(const InputPoints &xy, const IndexVector &rowPtr)
        {
#ifdef NOPYBIND
               // assume 2D arrays
               Index unitSize = 2;
    ...
    {
    public:
        PythonGeoQuery(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type);
        ~PythonGeoQuery() {}

        QueryResults nearest_to(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) { return _query->nearest_to(xy, rowPtr, type); }
        QueryResults intersects_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) { return _query->intersects_with(xy, rowPtr, type); }
        QueryResults overlaps_with(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type) { return _query->overlaps_with(xy, rowPtr, type); }
        QueryResults within_distance_of(const InputPoints &xy, const IndexVector &rowPtr, const std::string &type, Distance d) { return _query->within_distance_of(xy, rowPtr, type, d); }
