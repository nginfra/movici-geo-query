...
class GeoQueryWrapperBase
{
  public:
    ...
    virtual QueryResults nearest_to(const LocationArray &xy, const IndexArray &rowPtr,
                                    const std::string &type) = 0;
    virtual QueryResults intersects_with(const LocationArray &xy, const IndexArray &rowPtr,
                                         const std::string &type) = 0;
    virtual QueryResults overlaps_with(const LocationArray &xy, const IndexArray &rowPtr,
                                       const std::string &type) = 0;
    virtual QueryResults within_distance_of(const LocationArray &xy, const IndexArray &rowPtr,
                                            const std::string &type, Distance d) = 0;
};
...
class BoostGeoQueryWrapper : public GeoQueryWrapperBase
{
  public:
    ...
    QueryResults nearest_to(const LocationArray &xy, const IndexArray &rowPtr,
    ...
    QueryResults intersects_with(const LocationArray &xy, const IndexArray &rowPtr,
                                 const std::string &type)
    {
        if (type == accepted_input_types::point)
        {
        ...
        {
            return _boostQuery.intersects_with(
                _convert_geometry_for_query<OpenPolygon>(xy, rowPtr));
        }
        else if (type == accepted_input_types::closedPolygon)
        {
            return _boostQuery.intersects_with(
                _convert_geometry_for_query<ClosedPolygon>(xy, rowPtr));
        }
        else
        {
            throw std::invalid_argument("Input type not supported in BoostGeoQueryWrapper");
    ...
    QueryResults overlaps_with(const LocationArray &xy, const IndexArray &rowPtr,
                               const std::string &type)
    {
        if (type == accepted_input_types::point)
        {
        ...
        {
            return _boostQuery.overlaps_with(
                _convert_geometry_for_query<ClosedPolygon>(xy, rowPtr));
        }
        else
        {
    ...
    QueryResults within_distance_of(const LocationArray &xy, const IndexArray &rowPtr,
                                    const std::string &type, Distance d)
    {
        if (type == accepted_input_types::point)
        {
            return _boostQuery.within_distance_of(_convert_geometry_for_query<Point>(xy, rowPtr),
                                                  d);
        }
        else if (type == accepted_input_types::linestring)
        {
            return _boostQuery.within_distance_of(
                _convert_geometry_for_query<LineString>(xy, rowPtr), d);
        }
        else if (type == accepted_input_types::openPolygon)
        {
            return _boostQuery.within_distance_of(
                _convert_geometry_for_query<OpenPolygon>(xy, rowPtr), d);
        }
        else if (type == accepted_input_types::closedPolygon)
        {
            return _boostQuery.within_distance_of(
                _convert_geometry_for_query<ClosedPolygon>(xy, rowPtr), d);
        }
        else
        {
            throw std::invalid_argument("Input type not supported in BoostGeoQueryWrapper");
...
class PythonGeoQuery
{
  public:
    ...
    QueryResults nearest_to(const LocationArray &xy, const IndexArray &rowPtr,
                            const std::string &type)
    {
        return _query->nearest_to(xy, rowPtr, type);
    }
    QueryResults intersects_with(const LocationArray &xy, const IndexArray &rowPtr,
                                 const std::string &type)
    {
        return _query->intersects_with(xy, rowPtr, type);
    }
    QueryResults overlaps_with(const LocationArray &xy, const IndexArray &rowPtr,
                               const std::string &type)
    {
        return _query->overlaps_with(xy, rowPtr, type);
    }
    QueryResults within_distance_of(const LocationArray &xy, const IndexArray &rowPtr,
                                    const std::string &type, Distance d)
    {
        return _query->within_distance_of(xy, rowPtr, type, d);
    }
