...
namespace boost_geo_mapper
{
     ...
     class Vector2D
     {
          ...
          void push_back(const Point& entry) {_index.push_back(_data.size()); _data.push_back(entry);}
          void push_back(const PointVector& entries) {_index.push_back(_data.size()); _data.insert(_data.end(), entries.begin(), entries.end());}
