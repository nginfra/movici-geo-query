...
class RTreeGeometryQuery
{
  public:
    ...
    RTreeGeometryQuery() {}
    RTreeGeometryQuery(const TargetVector &target, const RTree &rTree)
        : _target_geo(target), _rTree(rTree)
    {}
    ...
    QueryResults overlaps_with(const std::vector<U> &geo)
    {
        return intersects_with(geo, true);
    }
  ...
  private:
    ...
    IndexVector intersects_with(const U &geo, bool require_full_overlap)
    {
        ...
        for (const auto &result : results)
        {
            ...
            if (bg::intersects(geo, target) &&
                (require_full_overlap ? !bg::touches(geo, target) : true))
            {
                rv.push_back(idx);
            }
        }
    ...
    DistanceResult nearest_to(const U &geo, int64_t initial_searchsize = -1)
    {
        ...
        while (!guaranteed_found)
        {
            ...
            for (; it != _rTree.qend(); ++it)
            {
                ...
                // then the other geometries are equal or further away and we found the nearest
                // geometry
            ...
            // if not sure we found the closest geometry (rather than closest box), get more
            // elements
