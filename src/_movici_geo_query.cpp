...
PYBIND11_MODULE(_movici_geo_query, m)
{
    ...
    py::class_<boost_geo_query::PythonGeoQuery>(m, "CGeoQuery")
        .def(py::init<boost_geo_query::LocationArray &, boost_geo_query::IndexArray &,
                      const std::string &>())
    ...
    py::class_<boost_geo_query::QueryResults>(m, "CQueryResults")
        .def("indices", [](const boost_geo_query::QueryResults &qr) { return qr.indices; })
        .def("row_ptr", [](const boost_geo_query::QueryResults &qr) { return qr.rowPtr; })
        .def("distances", [](const boost_geo_query::QueryResults &qr) { return qr.distances; });
}
