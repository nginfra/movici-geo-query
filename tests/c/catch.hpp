...
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_UNIQUE_PTR) &&                                       \
    !defined(CATCH_CONFIG_CPP11_NO_UNIQUE_PTR) && !defined(CATCH_CONFIG_CPP11_UNIQUE_PTR) && \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_UNIQUE_PTR
#endif
// Use of __COUNTER__ is suppressed if __JETBRAINS_IDE__ is #defined (meaning we're being parsed by
// a JetBrains IDE for analytics) because, at time of writing, __COUNTER__ is not properly handled
// by it. This does not affect compilation
#if defined(CATCH_INTERNAL_CONFIG_COUNTER) && !defined(CATCH_CONFIG_NO_COUNTER) && \
    !defined(CATCH_CONFIG_COUNTER) && !defined(__JETBRAINS_IDE__)
...
inline bool alwaysTrue(std::size_t = 0)
{
    return true;
}
inline bool alwaysFalse(std::size_t = 0)
{
    return false;
}
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                                  \
    ...
    } while (Catch::alwaysFalse(sizeof(expr)))  // expr here is never evaluated at runtime but it
